...
def summarize_values(values):
    ...
    if not values:
        return {"count": 0, "first": 0.0, "last": 0.0, "min": 0.0, "max": 0.0, "mean": 0.0, "delta": 0.0}
    return {
        "count": len(values),
        "first": values[0],
        "last": values[-1],
...
def discover_metric_csv_files(run_dir):
    ...
    # fallback for copied scenarios where export dirs were nested differently
...
def parse_metrics_snapshot(metric_csv_files):
    ...
    for svc, kpi_values in per_service_kpi_values.items():
        ...
        svc_metrics["memory_usage_last"] = _kpi(raw, "kpi_container_memory_usage_bytes", "container_memory_usage_bytes").get("last", 0.0)
        svc_metrics["network_rx_bytes_delta"] = _kpi(raw, "kpi_container_network_receive_bytes_total", "container_network_receive_bytes_total").get("delta", 0.0)
        svc_metrics["network_tx_bytes_delta"] = _kpi(raw, "kpi_container_network_transmit_bytes_total", "container_network_transmit_bytes_total").get("delta", 0.0)
        svc_metrics["network_tx_errors_delta"] = _kpi(raw, "kpi_container_network_transmit_errors_total", "container_network_transmit_errors_total").get("delta", 0.0)
        svc_metrics["network_rx_drops_delta"] = _kpi(raw, "kpi_container_network_receive_packets_dropped_total", "container_network_receive_packets_dropped_total").get("delta", 0.0)
        svc_metrics["network_tx_drops_delta"] = _kpi(raw, "kpi_container_network_transmit_packets_dropped_total", "container_network_transmit_packets_dropped_total").get("delta", 0.0)
        svc_metrics["threads_last"] = _kpi(raw, "kpi_container_threads", "container_threads").get("last", 0.0)
        svc_metrics["latency_ms"] = 0.0
        metrics[svc] = svc_metrics
    return {"per_service": metrics, "files_seen": files_seen, "parse_errors": parse_errors}


def parse_metrics_from_run_dir(run_dir):
    return parse_metrics_snapshot(discover_metric_csv_files(Path(run_dir)))
