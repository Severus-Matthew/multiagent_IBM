...
Infer the most likely upstream root cause from the telemetry. Separate root
...
class TrainingSafeLLMRCASolver:
    ...
    def solve(self, compressed_state: dict[str, Any], instruction: str) -> str:
        ...
        debug = {
            ...
            "postprocess_mode": "training_safe_no_candidate_repair_v2",
            "prompt_contains_candidate_menu": False,
            "training_safe_prompt_rules": [
                "default_single_root",
                "second_root_requires_independent_cluster",
                "unknown_discouraged_without_clear_component",
    ...
    def _build_user_prompt(self, agent_state: dict[str, Any], instruction: str, safety: dict[str, Any]) -> str:
        payload = {
            ...
            "root_cause_count_instruction": (
                f"Return at most {self.max_root_causes} line(s), but default to one. "
                "Use a second line only for independent evidence clusters."
            ),
...
def _truncate_state(state: dict[str, Any], char_budget: int) -> dict[str, Any]:
    ...
    if len(text) <= char_budget:
        return state
    keep_keys = ["system", "service_health", "llm_view", "traces", "metrics", "clusters", "graph", "sla", "redaction_note"]
